...
class BertForQuestionAnsweringWHL(BertPreTrainedModel):
    def __init__(self, config):
        ...
        # TODO check with Google if it's normal there is no dropout on the token classifier of SQuAD in the TF version
        # self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.weight_hidden = nn.Linear(4, 1)
        self.relu = nn.ReLU()
        self.qa_outputs = nn.Linear(config.hidden_size, 2)
        self.apply(self.init_bert_weights)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, start_positions=None, end_positions=None):
        ...
        #8 x 384 x 768 x 1
        sequence_output = self.weight_hidden(sequence_output)

        # 8 x 384 x 768
        sequence_output = self.relu(sequence_output.squeeze())
...
class Highway(nn.Module):
    def __init__(self, input_size):
        ...
        self.linear2 = torch.nn.Linear(input_size, input_size, bias = True)
        self.relu = nn.ReLU()
        self.sigmoid = nn.Sigmoid()

    def forward(self, x):
        """
        Forward functino should operate on batches.
        """
        x_proj = self.relu(self.linear1(x))
        x_gate = self.sigmoid(self.linear2(x))
        part1 = x_proj * x_gate
        part2 = (1 - x_gate) * x
...
class BertForQuestionAnsweringWHLHighway(BertPreTrainedModel):
    def __init__(self, config):
        ...
        # TODO check with Google if it's normal there is no dropout on the token classifier of SQuAD in the TF version
        self.highway = Highway(config.num_hidden_layers)
        self.dropout = nn.Dropout(self.hidden_dropout_prob)
        self.weight_hidden = nn.Linear(config.num_hidden_layers, 1)
        self.qa_outputs = nn.Linear(config.hidden_size, 2)
        self.apply(self.init_bert_weights)
...
class BertForQuestionAnsweringModifiedLoss(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, start_positions=None, end_positions=None):
        ...
        if start_positions is not None and end_positions is not None:
            ...
            weights =[2] + [1 for i in range(len(self.hidden_size) - 1)]
            weights = torch.tensor(weights, dtype = torch.float64, device = device).float()
